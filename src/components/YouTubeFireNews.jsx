...
const YouTubeFireNews = () => {
  ...
  const filteredNews = useMemo(() => {
    ...
    return (youtubeNewsData || []).filter((item) => {
      ...
      const strictFireWords = [
        '화재', '산불', '불길', '전소', '화마', '소방차', '소방관', '소방서', '소방대', '119출동', 
        '방화범', '불 탄', '불탄', '타올라', '타오른', '잔불', '발화', '소방당국', '소방본부'
      ];
      const fireContextPatterns = [
        /진화\s*(작업|헬기|완료|인력|작전|총력|율)/,
        /(불|불길|화마|산불|공장|건물|주택|아파트|차량|산|숲)\s*(을|를|이|가)?\s*진화/,
        /(가스|배터리|공장|충전기|보일러|변압기|유증기|폭죽|탱크|전기차)\s*(폭발|발화)/,
        /폭발\s*(사고|화재|음|으로|붕괴)/,
        /불이?\s*(나|붙어|번져|치솟|났|꺼졌|꺼진)/
      ];

      const hasStrict = strictFireWords.some(w => title.includes(w));
      const hasContext = fireContextPatterns.some(rx => rx.test(title));

      if (!hasStrict && !hasContext) {
        return false; // 실제 물리적 화재/소방 단어가 없으면 무조건 차단
      }

      // 비화재/은유/시사/비유 패턴 배제
      const nonFirePatterns = [
        /누리호.*진화/, /AI.*진화/, /기술.*진화/, /진화(하는|론|과정)/,
        /(분통|분노|불만|갈등|논란|사태|청와대|정부|여야|대통령실|트럼프).*(진화|폭발)/,
        /(인기|예능|드라마|주가|매출|수출|성장|반도체).*(폭발)/,
        /불순물/, /불법/, /불안/, /불출마/, /불통/, /불닭/, /불장난/, /마인크래프트/, /게임/,
        /파병/, /선거/, /총선/, /국회/, /의원/, /탄핵/, /검찰/, /특검/, /빚 갚아드려요/
      ];

      if (nonFirePatterns.some(rx => rx.test(title))) {
        return false;
      }
